...
install -Dm0644 /ctx/cosign.pub /etc/pki/containers/bioinformatico.pub

tmp=$(mktemp)
jq --arg ref "$IMAGE_REF" '.transports.docker[$ref] = [{
      type: "sigstoreSigned",
      keyPath: "/etc/pki/containers/bioinformatico.pub",
      signedIdentity: {type: "matchRepository"}
    }]' /etc/containers/policy.json > "$tmp"
install -m0644 "$tmp" /etc/containers/policy.json
rm -f "$tmp"

cat > "/etc/containers/registries.d/${registry}-bioinformatico.yaml" <<YAML
docker:
  ${IMAGE_REF}:
